...
public OnObjectMoved(objectid)
{
	...
	if(objectid == Obj_Elevator)   // The elevator reached the specified floor.
	{
	    ...
	    Label_Elevator	=
			CreateDynamic3DTextLabel("{CCCCCC}Нажмите '{FFFFFF}~k~~CONVERSATION_YES~{CCCCCC}' чтобы использовать", 0xCCCCCCAA, X_ELEVATOR_POS + 1.6, Y_ELEVATOR_POS - 1.85, z - 0.4, 4.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, 0, 0);
		    ...
		    if(pos[1] < (Y_ELEVATOR_POS - 1.81) && pos[1] > (Y_ELEVATOR_POS - 3.8) && pos[0] > (X_ELEVATOR_POS + 1.21) && pos[0] < (X_ELEVATOR_POS + 3.8))
		    {
				...
				if (i <= 12)
				{
				    ...
				    if (ElevatorState != ELEVATOR_STATE_MOVING)
				    {
				        ...
				        {
	                        SendClientMessage(playerid, COLOR_MESSAGE_YELLOW, "* Лифт находится на вашем этаже, войдите внутрь и нажмите '{FFFFFF}~k~~CONVERSATION_YES~{CCCCCC}'");
	                        return 1;
				        }
				    }
					...
					if (ElevatorState == ELEVATOR_STATE_MOVING)
					{
						...
						format(strTempString, sizeof(strTempString), "* Вы вызвали лифт, сейчас он движется на %s.", FloorNames[ElevatorFloor]);
					}
					else
					{
					    ...
					    {
					    	// Format chat text message
							format(strTempString, sizeof(strTempString), "* Вы вызвали лифт, сейчас он находится на %s.", FloorNames[ElevatorFloor]);
						}
						else
						{
					    	// Format chat text message
...
stock Elevator_Initialize()
{
	...
	Label_Elevator =
		CreateDynamic3DTextLabel("{CCCCCC}Нажмите '{FFFFFF}~k~~CONVERSATION_YES~{CCCCCC}' чтобы использовать", 0xCCCCCCAA, X_ELEVATOR_POS + 1.6, Y_ELEVATOR_POS - 1.85, GROUND_Z_COORD - 0.4, 4.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, 0, 0);
...
stock ShowElevatorDialog(playerid)
{
    ...
    ShowDialog(playerid, Show:<Elevator_Beachside>, DIALOG_STYLE_LIST, "Лифт", string, "Выбрать", "Отмена");
	return 1;
}
